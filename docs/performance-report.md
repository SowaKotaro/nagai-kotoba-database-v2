# パフォーマンス調査レポート（2026-08-06〜07、追記 2026-08-11）

データ量が増えてきて速度低下が気になる、という所感を起点に、**一般公開ページと管理者ページの
ボトルネックを実測**し、そのうち対処できたものを実装した記録。
「何が遅かったか」「なぜ遅かったか」「何をしたか」「まだ残っているか」を後から追えるようにまとめる。

> **本書は 2026-08 の記録として凍結してある**（2026-09-16 オーナー判断。`issues.md` 確定事項 35）。
> 以後の速度調査は本書に追記せず、別の文書に書く。
>
> **これは「その時点の実測の記録」であって、現在値の一覧ではない**。数値は 2026-08 の
> WSL 環境・当時のデータ量でのもので、本番の絶対値とも現在の値とも違う。
> **速度に手を付ける前に本書を読む**のは、同じ調査と同じ回り道を繰り返さないため
> （特に §3.2 の相関サブクエリ・§5.2 の GIL 占有・§7.5 の Levenshtein は、
> 直感で別の対処に向かいやすい）。対処は PR #121 ほかで本番へ反映済み。
> 再計測の手順は §1、残課題は §8。
