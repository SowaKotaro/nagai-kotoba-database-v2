...
## Issue 86: 関連語・しりとりが常に「最小 id の6件」を返す
- 種別: improvement
- 状態: 未着手
- 優先度: P1 ／ Impact: High ／ Effort: Low
- 依存: なし(**Issue 57 より先に直す**。軸を増やしても同じ顔ぶれが出るため)
- 背景・現状: 2026-09-17 の改善調査(A-3)より。`RelatedWords#build`(`related_words.rb:42`)と `ShiritoriWords#load_words`(`shiritori_words.rb:96-97`)がどちらも `order(:word_id).limit(6)` のため、該当語のうち **word_id が小さい6件**が必ず出る。公開語 1,467 件すべてで実際に引いたところ、関連語・しりとりのどちらかに一度でも出る語は **832 語(56.7%)** で、**635 語(43.3%)はどの単語詳細からもリンクされない**。語義7件以上の小分類ジャンル(279 中 51)では常に同じ6件が出て、新しく入れた語は永遠に出てこない。
- 内容:
  - [ ] 「決定的」を保ったまま語ごとに違う6件にする: **その語の id を起点に円環状にずらして取る**(`word_id > 自分の id` で6件 + 足りないぶんを先頭から、の2クエリならインデックスが効いたまま)
  - [ ] しりとりも同じ扱いにする(毎回同じ手が返ると遊びにならない)
  - [ ] 該当件数が6件以下のジャンルでは今までどおり全件が出ることをテストで縛る
- 期待効果: 語間の内部リンクの到達率 56.7% → ほぼ 100%。回遊(Issue 57 の背骨)が機能し、リンクが古い語に偏らなくなる。
- 補足: Issue 57 の「軸別ランダム遷移」とは別物(あちらは新しい導線、これは既存導線の修正)。

## Issue 87: 単語詳細の SQL 34 本を減らす(関連語・しりとりの preload 重複)
- 種別: improvement
- 状態: 未着手
- 優先度: P1 ／ Impact: Med ／ Effort: Low
- 依存: なし(Issue 86 と同じファイルを触るので、続けて行うと楽)
- 背景・現状: 2026-09-17 の改善調査(A-4)より。`/words/:id` が **34 本**の SQL を投げ、うち **22 本が `RelatedWords` の2グループ + `ShiritoriWords`**。グループごとに独立して `Word.where(id:).includes(...)` を実行する(`related_words.rb:45`・`shiritori_words.rb:101`)ため同じ preload が3回走り、ジャンルの祖先も1グループあたり3本出る。さらにビューの `.any?` → `.each` で `EXISTS` が1本余計に飛んでいる。
- 内容:
  - [ ] 各グループは `word_id` の配列だけを返し、呼び出し側で `Word.where(id: 全グループの id).includes(...)` を**1回**引いてグループへ配り直す(22 本 → 7 本程度)
  - [ ] ビューの `.any?` を読み込み済みの配列に対して呼び、`EXISTS` を消す
  - [ ] (任意)関連語・しりとりの区画のフラグメントキャッシュ化は Issue 97 で扱う
- 期待効果: サイトで最も叩かれるページの SQL を 1/3 に。クローラが詳細を舐めている間の DB 負荷がそのまま下がる。挙動は変わらない。
...
## Issue 93: ダークモードで「選択中」の面(--bg-tint)が見えない
- 種別: bug
- 状態: 未着手
- 優先度: P1 ／ Impact: Med ／ Effort: Low
- 依存: なし
- 背景・現状: 2026-09-17 の改善調査(B-3)より。`--bg-tint` ↔ `--surface` の差が**ライト ΔRGB 55 に対しダーク 11**で、design.md の下限 19 を割っている。`--dark-bg-tint: #1c2530` が `--dark-surface: #232930` より**暗い**ため面の上で沈む。`--bg-tint` は CSS 内 17 箇所(チップの選択状態・ジャンルピッカー・アノテーションの範囲タップの始点・デッキのドット・フォーカスリング `components.css:2345` など)で使われている。
- 内容:
  - [ ] `--dark-bg-tint` を `--dark-surface` より**明るい**側に取り直す(目安 `#283140`〜`#293240`、ΔRGB 29〜31。**19〜35 のあいだで最小**を採る)
  - [ ] **実測して決める**: 面どうしの ΔRGB と、`--dark-text` を載せたときのコントラスト(AA)を同時に確かめる
  - [ ] design.md の該当値を更新する
- 期待効果: ダークで「いま何を選んでいるか」が分かる。オーナーが最も長く見るアノテーション・コンソールにも効く。
...
- 依存: Issue 86(関連語・しりとりが常に同じ顔ぶれを返す問題を先に直す)
...
## Issue 96: 管理画面の認可をルート駆動のテスト1本で網羅する
- 種別: improvement
- 状態: 未着手
- 優先度: P2 ／ Impact: Med ／ Effort: Low
- 依存: なし
- 背景・現状: 2026-09-17 の改善調査(C-2)より。管理ルート 39 件を全数検査した結果、**認可の穴はゼロ**(すべて `Admin::BaseController` 配下で `require_authentication` が生きている)。一方テストは**未ログイン検証が 32 件**しかなく手書きなので、管理アクションを新しく足したときに書き忘れても気づけない。
- 内容:
  - [ ] `test/integration/` に、`Rails.application.routes.routes` から `admin/` 配下を列挙し、未ログインで全件 `new_session_path` へ送られることを確かめる1本を置く(動的セグメントはフィクスチャの id で埋める)
  - [ ] 各コントローラのテストにある個別の未ログイン検証を消す(CLAUDE.md「1つの振る舞いは1つの層で検証する」)
- 期待効果: 認可の網羅が構造的に保証され、テストの総数はむしろ減る。
- 補足: 破壊的アクションを含むので、認可が壊れていた場合はテスト DB のデータを消す(実害は無い)。
...
- 内容:
  - [ ] 単語詳細の関連語・しりとり区画(キー `[word, PublishedWordsDigest]`。Issue 86 の回転を入れた後)
