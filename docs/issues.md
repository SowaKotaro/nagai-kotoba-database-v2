...
- 内容: **2 段階で実装する。統計テーブルは先行して作らない**(統計は元データから常に再計算できる導出データのため)。
  - [x] **紙面構成・デザイン確定**(2026-07-18): コンセプト「蔵版目録」、数字の壁(4群×5指標)+8章構成。正は [`docs/stats.md`](stats.md)(モック Artifact へのリンクも同書に)
...
- 補足: データが数百語を超えてから公開する。看板ビジュアルは当初「五十音円環」(Issue 62)だったが 2026-07-18 に**不採用**とし、行×行ヒートマップ(§2)と級数見本ワードクラウド(§1、MeCab 事前集計が必要なため Phase 1.5)に置き換えた。
...
- 補足(2026-08-02): **Issue 75(収録リクエスト)で同じ総当たりが公開側の「重複チェック」ボタンから叩かれる**ようになる。Issue 75 側は (reading, surface) 一覧の `Rails.cache` 化と押下レート制限(1分10回目安)・比較10語上限で当座をしのぐが、本 Issue の絞り込みが入るまでは**管理者限定だった負荷が不特定多数から触れる状態**になる。データ量が増えるより先に、リクエスト機能の利用が伸びた時点で本 Issue を前倒しする。
...
- 種別: feature
- 状態: **不採用・クローズ**(2026-07-18 のデザイン検討で確定。パッと見で何を示すか読めず、頭文字→末尾文字の遷移をビジュアル化するメリットが伝わらないため。同じデータは行×行ヒートマップとして統計ページ §2 に吸収。正は [`docs/stats.md`](stats.md))
...
## Issue 75: 収録リクエスト(公開側からの単語リクエスト受付)
- 種別: feature
- 状態: 実装完了(2026-08-02。branch feature/word-requests。PR 作成待ち)
- 優先度: P2 ／ Impact: Med ／ Effort: Med
- 依存: なし(Issue 60 の「未発見募集」は将来この受け口に合流させる)
- 背景・現状: 2026-07-15 のアイデア出しで「要件を詰めてから」として保留していた件(確定事項 #12)。現在の受け口は About の連絡先メールのみ。2026-08-02 のオーナー回答で要件を確定した(確定事項 #19・#20)。**公開側で初めての POST エンドポイント**になるため、防御と逃げ道を要件に含める。
- 内容:
  - **スコープ**
    - [x] 受け付けるのは**収録リクエスト(語の追加)のみ**。既存語の訂正報告・機能要望は対象外(従来どおり About のメール)
    - [x] 投稿は**非公開**(管理者のみ閲覧)。公開側にリクエスト一覧は出さず、モデレーション義務を負わない
    - [x] **連絡先は受け取らない**(氏名・メール・ニックネームの欄を設けない。返信しない旨をフォームに明記)。ただし後述のとおり**リクエスト送信の技術メタデータは保存する**ため、「完全匿名」とは称さない
  - **公開側**
    - [x] ルーティングは `/requests/new` → `POST /requests`(`allow_unauthenticated_access`)。コントローラ名は `WordRequestsController`(`request` と紛らわしいため `RequestsController` にしない)
    - [x] 入口: (a) 検索結果0件の「見つかりませんでした」区画、(b) グローバルヘッダーのナビ(「検索」の次)、(c) フッター・About のお問い合わせ欄
    - [x] 検索0件からは**キーワード `q` をプリフィル**(`/requests/new?surface=...`)。ファセットのみの0件は語が特定できないため空フォームへ
    - [x] **フォームは専用ページに置き、0件ページに直接埋め込まない**。`words#index` 等は `public: true` の HTTP キャッシュ配下にあり、CSRF トークン入りフォームを共有キャッシュに載せるとトークンの共有・失効事故になる。リクエストページ自体には `expires_in`/`fresh_when` を付けない
    - [x] **1通につき最大10語**。行追加式(既存 Stimulus `nested-form` を流用)で、各行が 表層形(必須・100字)／読み(任意・100字)。既定は1行、10行を超えて追加できない
    - [x] 表の体裁にする(見出し行は 言葉／読み のみ。左端に行番号(CSS カウンタ)・右端に削除ボタン)。狭幅では1行 = 1枚の面に積んで横スクロールを作らない
    - [x] 読み欄に**字数カウンタ**(Stimulus)。10文字以上で「収録基準を満たします」を極小等幅で表示。**基準未満でも送信は弾かない**(警告のみ。読みの誤りで正当な語を落とさないため)
    - [x] 完了は POST 後リダイレクトでメッセージ表示(二重送信防止)
    - [x] `noindex, follow`。sitemap には載せない
    - [x] design.md 準拠(墨1px枠のパネル・朱は送信ボタンのみ・角丸/影なし。カード型フォームの影例外は適用しない)
    - [x] フォーム直下に「収録の検討にのみ利用します／返信はできません／収録をお約束するものではありません」を明記
  - **重複チェック(送信とは独立した任意操作)**
    - [x] フォームに**「重複チェックする」ボタン**を置く。**送信時には発火させない**(送信は常に受け付ける)
    - [x] 判定は既存の `Levenshtein`(`SIMILARITY_THRESHOLD = 0.8` を流用)。**読みが入力されていれば読み、無ければ表層形**に当てる。完全一致は距離計算の前に別途チェックして最優先で示す
    - [x] 押下中は**ローディング表示**(turbo-frame の busy 属性で照合中の行を沈める)
    - [x] 結果は**その語の入力欄の真下**に返す(結果用の列も別区画も作らない)。別区画を開くと「この内容で送る」が下へ押し出されて遠くなるため、差し替えるのは表の turbo-frame だけにする
    - [x] 既収録・類似は**朱枠**で引き止め(design.md: エラーは朱テキスト＋朱枠、背景は塗らない)、未収録は枠なしの薄墨で静かに伝える
    - [x] 判定は3通り。**いずれの場合も送信は可能**
      - 完全一致: 「すでに収録されています」＋その語の詳細へのリンク
      - 類似あり: 「似ている言葉があります」＋候補リンク
      - 該当なし: **「登録が確認されませんでした」**(未収録であることを伝え、リクエストの価値を実感させる。文言は ja.yml で調整可能に)
    - [x] エンドポイントは `POST /requests/duplicates`(管理側 `admin/words#duplicates` と同じ作法)。**レコードは作らない**
    - [x] **負荷対策**: 既存 (reading, surface) 一覧を `Rails.cache` に載せ(語数と最終更新をキー。llms.txt と同作法)、DB 総なめを毎回発生させない。比較対象は1回の押下につき最大10語
    - [x] **チェックにもレート制限**をかける(レコードを作らないため created_at カウントでは守れない。ここだけ `Rails.cache` の自前カウンタ。目安 1分10回)
    - [x] Issue 52(長さ帯での事前絞り込み)の対象範囲が公開側にも広がることを Issue 52 側に追記する
  - **スパム・荒らし対策(アプリ側のみ。外部サービス・追加 JS なし)**
    - [x] ハニーポット項目(CSS で隠した未使用欄)。埋まっていたら**成功レスポンスを返して黙って破棄**
    - [x] フォーム表示時刻を署名付き hidden(`MessageVerifier`)で持たせ、**3秒未満の送信は破棄**
    - [x] **レートリミットは `word_requests` の `ip_address` + `created_at` の COUNT で判定**(自前カウンタ不要)。目安 1時間5通 / 1日20通。`(ip_address, created_at)` にインデックス
    - [x] 文字数上限はモデル `validates` と DB の桁の両方で担保。1通 10語の上限もサーバ側で検証(クライアントの行数制御だけに頼らない)
    - [x] `REQUESTS_ENABLED`(既定 true)で受付を止められる。off ならフォームを出さず About のメール導線へ案内(`INDEXING_ENABLED` と同じ作法)
  - **管理側**
    - [x] `/admin/requests` 一覧(認証必須・新しい順・ステータス絞り込み)。行は**語単位**、送信メタデータは通単位で参照できる
    - [x] ステータス enum: **未着手 / 採用済み / 保留 / 却下**(既定 = 未着手)。理由は自由記述のメモ欄
    - [ ] **ステータスが変わるタイミング(手動か、登録完了で自動か)は TBD**。単語登録までの動線を確認してから再度詰める
    - [x] 各行に「同じ表層形が既に収録済みか」を表示(投稿後に登録された分の取りこぼし防止)
    - [x] **選択したリクエストを一括登録 step1 へ流し込む**(箇条書きに変換して `/admin/words/new` へ)。本機能の主動線
    - [x] 管理画面ヘッダに**未着手件数バッジ**＋管理コンソールに「リクエストの管理」カード(未着手件数つき)。通知メールは作らない(SMTP 基盤を増やさない)
    - [x] 物理削除ボタン(明らかな荒らし用)
    - [x] 同一 IP の投稿をまとめて見られる導線(荒らしの一括処理・解析用)
  - **データモデル(2テーブル)**
    - [x] `word_requests`(**送信1通**): `ip_address` / `user_agent` / `referer` / `origin_path`(送信元パス) / timestamps。メタデータは通単位で1回だけ持つ(語行に重複させない)。**レートリミットはこの行を数える**ため「1件を10回送信」と「10件を1回送信」を区別できる
    - [x] `word_request_items`(**語1件**): `word_request_id` / `surface`(NOT NULL) / `reading` / `status`(NOT NULL default 0 = 未着手) / `admin_memo` / `handled_at` / timestamps
    - [x] `surface`・`reading` のみ `utf8mb4_0900_as_ci`(読み・表層形まわりの例外規定どおり)、他は `0900_ai_ci`
    - [x] インデックス: `word_requests(ip_address, created_at)`、`word_request_items(status, created_at)`、`word_request_items.surface(191)`
    - [ ] 保持方針(削除・匿名化の期限)は解析用途と両立する範囲で決める → **TBD**
  - **法務・文言**
    - [x] **プライバシーポリシーの改訂が必須**。リクエストフォームで IP アドレス・ブラウザの種類(User-Agent)・リファラー・送信元パスを**保存**すること、利用目的(不正投稿の防止・サービス改善のための分析)を明記する。既存のアクセスログの記載(`log_body`)とは別項目として書く
    - [x] フォーム直下にも要約を1文置き、送信前に分かるようにする
    - [x] 表示文言は `ja.yml` に集約
  - **テスト**
    - [x] モデル: 文字数の境界・1通10語の上限・ステータス
    - [x] 結合: 未認証で投稿できる／管理画面は要認証／ハニーポット・時間トラップが効く／IP+created_at のレートリミットが効く／**重複があっても送信できる**／`REQUESTS_ENABLED=false` で受付停止／メタデータが保存される
    - [x] 重複チェック: 完全一致・類似あり・該当なしの3分岐／読み無しでも表層形で判定される／チェック自体のレート制限
    - [x] システム: 検索0件 → プリフィル → 重複チェック → 送信完了
- 期待効果: 収録の種を外から集める。検索0件という「最も欲しい瞬間」を受け止め、離脱をデータ源に変える。重複チェックの「登録が確認されませんでした」で利用者に発見の手応えを渡し、送信メタデータは今後の導線改善の分析材料になる。Issue 60 の未発見枠の受け皿にもなる。
- 補足: 反応が乏しければ入口を畳むだけで撤退できる(受付停止は `REQUESTS_ENABLED` で、導線ごと隠れる)。
...
- **Issue 73: llms-full.txt(全収録データの全文版)** [feature] — 完了。`GET /llms-full.txt` で公開語を1ファイルに。1語 = 見出し語 / URL / 文字種 / 定義文 / 語義ごとの読み・字数・拍数・先頭末尾文字・意味・ジャンル階層・品詞・エンティティ・語種・言語学的特徴・別表記(語義が複数の語だけ「#### 語義N」で区切る)。冒頭に読み方・収録基準・CC BY 4.0 のクレジットを置き、引用時に出典表記がセットで渡るようにした。本文は `Rails.cache`(語数と最終更新をキー)+ HTTP 1日キャッシュ、ビューは `find_each`。約1700語で約900KB・生成1秒弱のため、**数万語規模になったら分割(ジャンル別・文字数上限)を検討する**。
