...
- 内容:
  - [ ] **特徴だけを対象にした調査用 JSON の書き出し**(対象語の表層形・読み・現在の特徴 + 特徴マスタ一覧。意味やジャンルは載せない)
  - [ ] 対応する**取り込み**(提案は既存どおり `annotation_proposals` の下書きに入れ、承認は人間が行う)
  - [ ] **「特徴なし」がオーナーの判断結果である語を、再提案で蒸し返さない**仕組み。付与率が低い語の中には「調べたうえで該当なしと判断した」ものが混じる(例: Ⅲ を「サン」と読むのは特殊読みではない、というオーナー判断)。**未調査**と**調査済みで該当なし**を区別できないと、同じ語を何度も突き返すことになる
  - [ ] 対象語の絞り込み(特徴が0件の公開語、日本語を含む語を優先 など)
