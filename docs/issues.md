...
- 内容:
  - [ ] ローカルで MeCab を回して形態素頻度を集計する冪等な rake タスク(`backfill:reading_metrics` と同じ作法)
  - [ ] 集計結果の置き場所を決める(コミットするデータファイル / 専用テーブルのいずれか。本番は読むだけにして MeCab 依存を持ち込まない)
  - [ ] `docs/design.md` 準拠の描画(墨・朱のみ。チャートライブラリ不使用)
  - [ ] 語をタップしたらその形態素を含む語の検索へ(design.md §5.5 の「関連データは検索の絞り込みへの導線にする」)
