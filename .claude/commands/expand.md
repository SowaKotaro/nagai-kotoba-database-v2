...
argument-hint: "[任意: 収集軸（例 キャラクター名）／入力ファイルパス。既定は種語と同じ軸・research/inputs/expansion.txt]"
---
...
- 引数の解釈: 引数が**ファイルパスに見えるなら入力パス**として使う。**それ以外の語句は収集軸**
  （例 `キャラクター名` `楽曲名` `必殺技名`）として使い、種語と同じ軸ではなくその軸で収集する。
  空なら既定（入力 `research/inputs/expansion.txt`／収集軸は種語と同じ軸）。
- 出力ファイル: `research/outputs/expansion.txt`（上書き）。
- `.claude/skills/word-expansion-research/SKILL.md` の手順に厳密に従うこと
  （上位概念と収集軸の特定 → 実在する一覧を出典に列挙 → 4原則・明らかに短い語でふるい分け →
