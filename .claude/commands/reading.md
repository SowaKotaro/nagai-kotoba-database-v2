...
argument-hint: "[任意: 入力ファイルパス。既定 inputs_reading_tmp.txt]"
---
...
- 引数: $ARGUMENTS
- 入力ファイル: 上の引数が空でなければそれを入力パスに使う。空なら既定の `inputs_reading_tmp.txt`。
- 出力ファイル: `outputs_reading_tmp.json`（上書き。step2 の取り込み欄にそのまま貼れる JSON）。
