...
argument-hint: "[任意: 入力ファイルパス。既定 research/inputs/annotation.json]"
---
...
- 引数: $ARGUMENTS
- 入力ファイル: 上の引数が空でなければそれを入力パスに使う。空なら既定の `research/inputs/annotation.json`
  （管理画面「調査用データの書き出し」の JSON を保存したもの）。
- 出力ファイル: `research/outputs/annotation.json`（上書き。管理画面「提案 JSON の取り込み」にそのまま貼れる）。
