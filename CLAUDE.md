...
- **UI を作る/変更する、または雰囲気・トーンを確認するときは、必ず [`docs/design.md`](docs/design.md) を読む**。本プロジェクトのデザインシステム（「紙とインク＝図鑑・辞典の手触り」）の確定仕様。
- 要点: 生成りの紙＋墨のパレット（CSS 変数のトークンのみ使用。紫/青系・グラデーション禁止）／**角丸・影・ピル型タグは全面禁止**（白基調＋角丸＋薄影の「AIデザイン」は試作の結果**不採用**と決定済み。再提案しない）／見出しは明朝・本文は OS 標準ゴシック（web フォントは明朝1本のみ）／主要コンテンツは**台紙カード**（`--paper-card`＋1px 罫線）、メタ情報は 1px 罫線グリッドのラベル表／**朱は該当部分の注釈とエラー専用**／アイコンは**インライン SVG**（絵文字・アイコンフォント禁止）。
- **関連データ（ジャンル・品詞・エンティティ・特徴）は雑に並べない**: 必ず専用コンポーネント（リンク付きパンくず／四角タグ／苔チップ）にし、**検索の絞り込みへの導線**にする（`docs/design.md` §5.3）。
- CSS は手書き（Sprockets、`tokens → base → layout → components`）。**パフォーマンス最優先**で、デザインのために重いフォント/CDN/過剰な JS を入れない（`font-display: swap`・画像遅延・importmap 維持）。
