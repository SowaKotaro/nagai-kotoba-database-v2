...
- **UI を作る/変更する、または雰囲気・トーンを確認するときは、必ず [`docs/design.md`](docs/design.md) を読む**。本プロジェクトのデザインシステム「**活字見本帖（Type Specimen）**＝言葉そのものが主役」の確定仕様。
- 要点: 紙（温かい白）×墨（ほぼ黒）の高コントラスト＋**朱一色だけのアクセント**（CSS 変数のトークンのみ使用。第二のアクセント色・紫/青系・グラデーション禁止）／**言葉は巨大な明朝、データラベルは極小の等幅**のスケール対比／構造は墨の太罫・細罫・**墨 1px 枠のパネル**で作る（**角丸・影・ピル型は全面禁止**。「AIデザイン」もベージュの「古書風」も試作の結果不採用と決定済み。再提案しない）／フッターは墨のベタ面／hover は墨反転か朱／アイコンはインライン SVG（絵文字・アイコンフォント禁止）。
- **関連データ（ジャンル・品詞・エンティティ・特徴）は雑に並べない**: 必ず専用コンポーネント（朱区切りのパンくず／墨枠タグ・チップ）にし、**検索の絞り込みへの導線**にする（`docs/design.md` §5.5）。
- CSS は手書き（Sprockets、`tokens → base → layout → components`）。**パフォーマンス最優先**で、デザインのために重いフォント/CDN/過剰な JS を入れない（web フォントは明朝1本・`font-display: swap`・画像遅延・importmap 維持）。
